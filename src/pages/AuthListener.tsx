      ...
      <div className="w-full max-w-[420px] bg-[#141418]/85 backdrop-blur-[24px] saturate-180 border border-white/10 rounded-[24px] p-8 md:p-10 relative z-10 mx-auto shadow-2xl">
        <div className="text-center mb-8">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#00A3FF]/10 border border-[#00A3FF]/20 rounded-full mb-3">
            <Headphones size={16} className="text-[#00A3FF]" />
            <span className="text-[#00A3FF] text-xs font-semibold font-semiboldst">
              Listener Portal
            </span>
          </div>
           ...
           {mode === 'login' ? (
              <motion.form key="login" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} onSubmit={handleLogin} className="space-y-4">
                 ...
                 <div className="relative flex items-center py-4">
                    ...
                    <span className="flex-shrink-0 mx-4 text-text-muted text-[11px] font-display font-medium font-semiboldst">or</span>
                    <div className="flex-grow border-t border-white/10"></div>
                 </div>
               ...
               <motion.form key="signup" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} onSubmit={handleSignup} className="space-y-4">
                 ...
                 {false && (
                   <div className="space-y-3">
                     ...
                     <input
                       ...
                       className="w-full h-[52px] text-center text-2xl font-mono tracking-[0.5em] bg-white/5 border border-white/10 rounded-[12px] text-white focus:outline-none focus:border-[#00A3FF]/60 focus:ring-[3px] focus:ring-[#00A3FF]/12 transition-all"
                     />
                     <button
                       type="button"
                       onClick={verifyOtp}
                       disabled={otpLoading || otpCode.length < 4}
                       className="w-full h-[52px] rounded-[12px] font-display font-bold text-[15px] font-semibold text-white transition-all hover:brightness-110 disabled:opacity-50"
                 ...
                 <button type="submit" disabled={loadingState || otpLoading} className="w-full h-[52px] rounded-[12px] font-display font-bold text-[15px] font-semibold text-white shadow-sm transition-all hover:brightness-110 hover:scale-[1.01] active:scale-[0.98] mt-2" style={{ background: '#0084D6' }}>
