        ...
        if (pcStatus === "successful" || pcStatus === "success") {
            try {
              if (dbTx.status !== 'completed') {
                const simulatedPayload = payload.data;
                if (!simulatedPayload.tx_ref) simulatedPayload.tx_ref = tx_ref;
                simulatedPayload.status = "successful";
                
                await fetch(`http://127.0.0.1:${PORT}/api/paychangu-webhook`, {
                  method: 'POST',
               ...
               console.error("Local webhook trigger failed:", e);
            }
