...
const fixCors = (filePath) => {
  ...
  const corsRegex = /const _ALLOWED_ORIGINS = \[[\s\S]*?\];\s*function getCorsHeaders\(origin[^)]*\) {[\s\S]*?return {[\s\S]*?};\s*}/g;
  ...
  if (corsRegex.test(content)) {
    ...
    const fallbackRegex = /export const corsHeaders = {[\s\S]*?};/g;
    // but they use getCorsHeaders now
